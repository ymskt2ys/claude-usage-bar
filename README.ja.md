...
- Xcode Command Line Tools (`swiftc` が使えればいい。Xcode 本体は不要)
- Claude の有料プラン。表示するのはプランの使用率なので、従量課金の API キー利用では出ない

枠ごとに前提が違う。

| 枠 | 前提 |
|---|---|
| 5時間枠・週(全体) | **Claude デスクトップアプリ**を使っていること。このアプリが使用率をローカルに記録する |
| 週(モデル別) | **Claude Code CLI にログイン済み**であること。Keychain の OAuth 資格情報を使って API から取る |

どちらも無い場合は `~/.claude.json` のキャッシュに落ちるが、これは Claude Code CLI が動いた時しか
更新されない。

## 配布について

ビルドした `.app` は ad-hoc 署名 (`codesign -s -`) なので、**ダウンロードして配ると Gatekeeper に止められる**。
自分でビルドした場合は quarantine 属性が付かないのでそのまま起動できる。
配布するには Apple Developer Program と notarization が必要。
...
どの枠がどのソースから来たか、API が失敗していればその理由が出る。
...
Xcode プロジェクトは不要。`swiftc` で 2 ファイルをアーキごとに直接コンパイルし、`lipo` で束ねて
`.app` を組み立てる。`LSUIElement` を立てているので Dock にアイコンは出ない。
...
これらは公開 API ではない。Anthropic 側の変更で予告なく壊れる。
...
| `build.sh` | universal な `.app` バンドル生成 + ad-hoc 署名 |
